...
def verilog2doc(verilogs, top=None, output=None):
    ...
    patternModule = re.compile(
        r"module\s+(?P<name>\w+)(?P<params>\s*#\([^\)]*\))?\s*\((?P<args>[^\)]*)\)\s*;(?P<data>[\s\S]*?(?=endmodule))endmodule"
    )
    patternParam = re.compile(
        r"parameter(?P<type>\s+[a-zA-Z]+)?(?P<size>\s*\[.*\])?(?P<name>\s+\w+)\s*(?P<default>=.*)"
    )
    patternArg = re.compile(
        r"(?P<dir>output|input|inout)?(?P<type>\s+reg|wire)?(?P<size>\s\[[^\]]*\])?(?P<name>\s\w+)"
    )
    ...
    if output is None:
        if "/" in modules[top]["filename"]:
            output = (
                "/".join(modules[top]["filename"].split("/")[0:-1]) + "/Documentation"
            )
        else:
    ...
    def dependsGraph2menu(fd, dependsGraph, prefix=""):
        for module in dependsGraph:
            ...
            fd.write(
                f"{prefix}<a target='main' href='{filename.split('/')[-1]}.html#{module}'>{module}</a><br/>\n"
            )
        ...
        for moduleName, module in modules.items():
            ...
            if graphviz is not None:
                    ...
                    for sub in module["sub"]:
                        if sub[0] in modules:
                            ...
                            for argName, arg in modules[sub[0]]["args"].items():
                                if not arg.get("defines"):
                                    sports.append(
                                        f"<{argName}>{argName}{arg.get('size') or ''}"
                                    )
                            label = f"{{ {sub[0]} | {{ {' | '.join(sports)} }} }}"
                            c.node(
                                f"{moduleName}_{sub[0]}",
                                shape="record",
                                label=label,
                                fontsize="11pt",
                            )
                ...
                gAll.node(
                    ...
                    shape="plaintext",
                    label=moduleName,
                    href=f"{filename.split('/')[-1]}.html#{moduleName}",
                )

            fd.write("<h3>arguments</h3>\n")
            fd.write("<table>\n")
            fd.write(
                "<tr><th>direction</th><th>type</th><th>size</th><th>name</th></tr>\n"
            )
            for argName, arg in module["args"].items():
                fd.write(
                    f"<tr><td>{arg.get('direction', '')}</td><td>{arg.get('type', '')}</td><td>{arg.get('size', '')}</td><td>{argName}</td><td>{arg.get('defines') or ''}</td></tr>\n"
                )
            fd.write("</table>\n")
            fd.write("<br>\n")
            ...
            if module["params"]:
                ...
                for argName, arg in module["params"].items():
                    fd.write(
                        f"<tr><td>{arg.get('size', '')}</td><td>{arg.get('name', '')}</td><td>{arg.get('default', '')}</td></tr>\n"
                    )
                fd.write("</table>\n")
                fd.write("<br>\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", "-o", help="output directory", type=str, default=None
    )
