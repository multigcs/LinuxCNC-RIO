...
typedef struct {
    ...
    hal_float_t *spindle_speed_fb_rps[NUM_VFD_SPINDLES];
...
int modbus_hyvfd_init(int comp_id, const char *prefix) {
    ...
    *(mb_data->spindle_speed_fb[spindle]) = 0.0;
    ...
    retval = hal_pin_float_newf(HAL_OUT, &(mb_data->spindle_speed_fb_rps[spindle]), comp_id, "%s.MODBUS_HYVFD_%i.spindle-speed-fb-rps", prefix, spindle);
    if (retval < 0) return -1;
    *(mb_data->spindle_speed_fb_rps[spindle]) = 0.0;
