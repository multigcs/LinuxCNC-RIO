    ...
    def pinlist(self):
        ...
        for num, data in enumerate(self.jdata["plugins"]):
            if data.get("type") == self.ptype:
                ...
                pinlist_out.append(
                    (f"PICOSOC{num}_FLASH_IO0", data["pins"].get("flash_io0") or data["pins"]["flash_mosi"], "INOUT", pullup)
                )
                pinlist_out.append(
                    (f"PICOSOC{num}_FLASH_IO1", data["pins"].get("flash_io1") or data["pins"]["flash_miso"], "INOUT", pullup)
                )
