...
for num, vout in enumerate(project["voutnames"]):
    if vout.get("type") == "vout_sine":
        ...
        voutminmax.append(
            (vout.get("min", -100), vout.get("max", 100), "rcservo", freq)
        )
        ...
        try:
            ...
            for vn, value in enumerate(vouts):
                if voutminmax[vn][2] == "sine":
                    ...
                    value = int(
                        (value) * (PRU_OSC / voutminmax[vn][3]) / (voutminmax[vn][1])
                    )
                elif voutminmax[vn][2] == "pwm":
                    value = int(
                        (value - voutminmax[vn][0])
                        * (PRU_OSC / voutminmax[vn][3])
                        / (voutminmax[vn][1] - voutminmax[vn][0])
                    )
